...
parser.add_argument('input', default='./flowers/test/1/image_06754.jpg',
...
def main():
    model = modelX.load_checkpoint(path)
    with open(category_file, 'r') as json_file:
        cat_to_name = json.load(json_file)

    probabilities = modelX.predict(
        path_image, model, number_of_outputs, device)
    labels = [cat_to_name[str(index + 1)]
